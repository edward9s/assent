...
## Automatic bounded workflow consensus

The agreed workflow has three ordered arrays: task, plan, and integration. Their mechanical actions are `focused_test`, `focused_sweep`, and `full_verify`. A pass completes the corresponding layer without reviewer confirmation; a failure advances to configured reviewer/fixer work and a later action rechecks it. `run` performs this automatically, while `accept` remains the explicit human decision.

Scope omission is an AI-manageable repair case when the reviewer returns one exact path and the scheduler validates and applies it. Review prompts state the current and total rounds, require convergence, and forbid findings not grounded in an existing requirement or concrete repair regression.

The arrays themselves are the finite bound. No no-progress or diff-oscillation heuristic is used. Exhaustion preserves every edit and finding, reports `REVIEW UNRESOLVED, HUMAN DECISION`, and exits zero so unrelated work continues; mechanical verification evidence still controls whether acceptance is allowed.
