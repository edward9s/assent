...
5. **無人值守完成,人類裁決**
   調度器把自己能決定的全部決定完,無法決定的則以 report 證據的形式送到
   人類驗收會議。兩者的分界是「這個決定需不需要人」,不是「程式碼寫完了沒」。
   落到操作上就是行程 exit code:`run --all` 一旦有任何 folder 以非零結束,
   啟動迴圈就不再啟動新的 folder(`folder_scheduler.py` 的啟動迴圈以該
   failure 旗標為守衛),所以一個非零 exit 會無聲地取消佇列中不相干的工作。
   因此調度器無法裁決的問題 —— 未經確認的自我修復、未解決的 review
   finding —— 該 folder 要以 exit 0 結束,配一個獨立命名的 `_report.md`
   狀態與 `accept` 時的明確閘門:folder 帶著一個未決問題抵達會議,而不是
   把整個佇列一起拖下去。非零只保留給真正的失敗 —— 基礎設施、被拒絕的前置
   條件、壞掉的 gate —— 也就是繼續下去並不安全,而不只是尚未裁決的情況。
