"""Ignored-input decisions, provisioning, and verification evidence.

Every case here runs against a disposable Git repository with real worktrees and
real links -- directory junctions or symlinks and file hard links or symlinks --
because the feature's contract is what happens on a filesystem: which
...
from assent.ignored_inputs_cli import ignored_inputs_declare, ignored_inputs_status
...
def settle_ignored_inputs(main: Path, worktree: Path, *required: str) -> None:
    """Record one reviewed ignored-input profile so verification may start.
    ...
    ignored-input gate. With no required input this records the reviewed empty answer,
    which is what those repositories honestly are. A complete verification
    consumer still refuses any input link those low-level cases create by
    ...
    ignored_inputs.declare(main, Path(worktree), required=required,
                        watch=tracked[:1], none_required=not required,
                        not_required=excluded_input_inventory(main, required))


def excluded_input_inventory(
        main: Path, required: tuple[str, ...] | list[str] = ()
        ) -> tuple[ignored_inputs.NonRequiredInput, ...]:
    """Keep unrelated fixtures explicit without repeating their inventory."""
    selected = set(required)
    return tuple(
        ignored_inputs.NonRequiredInput(
            relative, "not required by this focused test")
        for relative in ignored_inputs.ignored_inventory(Path(main))
        if not any(relative == root or relative.startswith(f"{root}/")
                   for root in selected))


class IgnoredInputsCase(unittest.TestCase):
    """One repository, one source worktree, and real ignored inputs."""
    ...
    def setUp(self) -> None:
        ...
        (self.root / ".env").write_text("TOKEN=local-secret\n", encoding="utf-8")
    ...
    def _review(self, *required: str, none_required: bool = False,
        ...
        return ignored_inputs.declare(self.root, self.worktree, required=required,
    ...
    def _classify(self, worktree: Path | None = None, **kwargs):
        return ignored_inputs.classify(
...
class TestDecisionStates(IgnoredInputsCase):
    ...
    def test_review_accepts_a_wholly_ignored_tree_when_its_name_is_not_ignored(self):
        ...
        self.assertEqual(contract.state, ignored_inputs.REVIEWED_REQUIRED)
    ...
    def test_manifest_has_one_unversioned_feature_specific_format(self):
        self._review("pkg")
        manifest_path = ignored_inputs.manifest_path(self.root)
        self.assertEqual(manifest_path.name, "_ignored-inputs.toml")
        text = manifest_path.read_text(encoding="utf-8")
        self.assertNotIn("version", text)
        self.assertIn("[[ignored_inputs.profile]]", text)
    ...
    def test_settled_is_pure_when_an_undeclared_link_appears(self):
        ...
        self.assertEqual(ignored_inputs.closeout_refusal(contract), "")
    ...
    def test_same_primary_orphan_is_reviewed_before_manifest_mutation(self):
        ...
        self.assertEqual(settled.state, ignored_inputs.REVIEWED_REQUIRED)

        reviewable = ignored_inputs.review_decision_with_source_links(
            self.root, self.worktree, settled)
        self.assertEqual(reviewable.state, ignored_inputs.STALE)
        self.assertTrue(reviewable.needs_review)
        self.assertIn("lib/l10n/arb", "\n".join(reviewable.evidence))
        before = ignored_inputs.manifest_path(self.root).read_bytes()

        prepared = ignored_inputs.prepare_worktree(self.root, self.worktree)
        self.assertEqual(prepared.state, ignored_inputs.STALE)
        with self.assertRaisesRegex(
                AssentError, "omitted existing ignored-input link.*lib/l10n/arb"):
            ignored_inputs.validate_declaration(
                self.root, self.worktree, ("pkg",), ("pubspec.yaml",),
                excluded_input_inventory(self.root, ("pkg",)))
        self.assertEqual(
            ignored_inputs.manifest_path(self.root).read_bytes(), before)

        validated = ignored_inputs.validate_declaration(
            self.root, self.worktree, ("pkg", "lib/l10n/arb"),
            ("pubspec.yaml",),
            excluded_input_inventory(self.root, ("pkg", "lib/l10n/arb")))
    ...
    def test_two_branch_fingerprints_reuse_their_own_profiles_without_oscillation(
        ...
        back = self._classify()
        self.assertEqual(back.state, ignored_inputs.REVIEWED_REQUIRED)
        self.assertEqual(back.required, ("pkg",))
        self.assertNotEqual(first.profile.fingerprint, second.profile.fingerprint)
        manifest = ignored_inputs.read_manifest(self.root)
        self.assertEqual(
    ...
    def test_conflicting_matching_profiles_fail_closed(self):
        ...
        clone = ignored_inputs.Profile(
            base.fingerprint, ("assets",), base.watch, dict(base.digests),
            base.inventory, excluded_input_inventory(self.root, ("assets",)),
            base.kinds)
        manifest.profiles = manifest.profiles + (clone,)
        ignored_inputs.write_manifest(self.root, manifest)
    ...
    def test_one_review_replaces_all_matching_answers_only(self):
        ...
        manifest = ignored_inputs.read_manifest(self.root)
        readme_digests = ignored_inputs.snapshot_digests(
            self.worktree, ("README.md",))
        conflicting = ignored_inputs.Profile(
            ignored_inputs.fingerprint_of(readme_digests), ("assets",),
            ("README.md",), readme_digests,
            manifest.profiles[0].inventory,
            excluded_input_inventory(self.root, ("assets",)),
            manifest.profiles[0].kinds)
        stale_digests = dict(readme_digests)
        stale_digests["README.md"] = "0" * 64
        retained = ignored_inputs.Profile(
            ignored_inputs.fingerprint_of(stale_digests), ("lib/l10n/arb",),
            ("README.md",), stale_digests,
            manifest.profiles[0].inventory,
            excluded_input_inventory(self.root, ("lib/l10n/arb",)),
            manifest.profiles[0].kinds)
        ...
        profiles = ignored_inputs.read_manifest(self.root).profiles
        self.assertEqual(len(profiles), 2)
        self.assertIn(retained, profiles)
        self.assertEqual(sum(profile.required == ("pkg",) for profile in profiles), 1)


class TestNoIgnoredInputCandidate(IgnoredInputsCase):
    ...
    def test_only_a_successful_empty_discovery_settles_the_fast_path(self):
        ...
        self.assertFalse(ignored_inputs.has_ignored_input_candidate(bare))
        ...
        self.assertFalse(ignored_inputs.has_ignored_input_candidate(bare))
        self.assertEqual(ignored_inputs.classify(bare, bare).state,
                         ignored_inputs.NO_IGNORED_INPUT_CANDIDATE)

    def test_an_ignored_leaf_file_is_an_input_candidate(self):
        bare = self._plain_repository()
        ...
        self.assertTrue(ignored_inputs.has_ignored_input_candidate(bare))
        contract = ignored_inputs.classify(bare, bare)
        self.assertEqual(contract.state, ignored_inputs.UNKNOWN)
        self.assertEqual(contract.inventory, ("model.g.dart",))
        self.assertEqual(contract.kinds, {"model.g.dart": "file"})
    ...
    def test_an_existing_candidate_still_counts_when_it_reviews_to_none(self):
        ...
        self.assertTrue(ignored_inputs.has_ignored_input_candidate(self.root))
        self._review(none_required=True)
        contract = self._classify()
        self.assertEqual(contract.state, ignored_inputs.REVIEWED_NONE)
    ...
    def test_a_failed_ignored_entry_query_refuses_instead_of_answering_none(self):
        ...
        with self.assertRaises(AssentError):
            ignored_inputs.has_ignored_input_candidate(broken)
        with self.assertRaises(AssentError):
            ignored_inputs.classify(broken, broken)


    def test_required_evidence_without_a_primary_target_refuses_precisely(self):
        bare = self._plain_repository()
        with self.assertRaises(AssentError) as missing:
            ignored_inputs.classify(bare, bare, required_evidence=("pkg",))
        self.assertIn("does not exist in the primary worktree",
                      str(missing.exception))
        self.assertIn("ignored-inputs declare", str(missing.exception))
    ...
    def test_a_source_only_directory_is_not_a_provisionable_primary_target(self):
        ...
        self.assertTrue(
            ignored_inputs.has_ignored_input_candidate(self.worktree))
    ...
    def test_the_fast_path_settles_every_gate_without_a_manifest(self):
        bare = self._plain_repository()
        contract = ignored_inputs.prepare_worktree(bare, bare)
        self.assertEqual(contract.state,
                         ignored_inputs.NO_IGNORED_INPUT_CANDIDATE)
        self.assertEqual(ignored_inputs.closeout_refusal(contract), "")
        prepared = ignored_inputs.prepare_sources(bare, [("plan01", bare)])
        ...
        self.assertTrue(ignored_inputs.ignored_inputs_digest(bare, prepared))
        self.assertEqual(ignored_inputs.read_manifest(bare).profiles, ())


class TestDeclareOperation(IgnoredInputsCase):
    def test_every_unsafe_value_is_refused_before_any_mutation(self):
        ...
        self.assertFalse(ignored_inputs.manifest_path(self.root).exists())
    ...
    def test_overlapping_declarations_are_refused(self):
        ...
        with self.assertRaisesRegex(AssentError, "overlapping required inputs"):
    ...
    def test_an_omitted_inventory_entry_cannot_be_recorded(self):
        ...
        not_required = excluded_input_inventory(self.root, ("pkg",))
        contract = ignored_inputs.declare(
            self.root, self.worktree, required=("pkg",),
            watch=("pubspec.yaml",), not_required=not_required)
        reread = ignored_inputs.read_manifest(self.root).profiles[-1]
    ...
    def test_a_primary_inventory_change_is_reconsidered_before_the_next_session(self):
        ...
        contract = self._classify()
        self.assertEqual(contract.state, ignored_inputs.STALE)
        self.assertIn("ignored input added: build",
                      contract.evidence)

    def test_a_second_concurrent_review_is_refused_not_interleaved(self):
        self._review("pkg")
        before = ignored_inputs.manifest_path(self.root).read_bytes()
        with ignored_inputs.hold_manifest_lock(self.root):
            with self.assertRaisesRegex(AssentError, "already updating"):
                self._review("assets")
        self.assertEqual(ignored_inputs.manifest_path(self.root).read_bytes(),
                         before)

    def test_a_malformed_manifest_refuses_rather_than_reading_as_empty(self):
        ignored_inputs.manifest_path(self.root).parent.mkdir(
            parents=True, exist_ok=True)
        ignored_inputs.manifest_path(self.root).write_text(
            "not = [toml\n", encoding="utf-8")
        with self.assertRaisesRegex(AssentError, "not valid TOML"):
            ignored_inputs.read_manifest(self.root)
        ignored_inputs.manifest_path(self.root).write_text(
            "unexpected = true\n", encoding="utf-8")
        with self.assertRaisesRegex(AssentError, "only top-level table"):
            ignored_inputs.read_manifest(self.root)
        ignored_inputs.manifest_path(self.root).write_text(
            '[[ignored_inputs.profile]]\nfingerprint = "a"\n'
            'required = ["../escape"]\n', encoding="utf-8")
        with self.assertRaises(AssentError):
            ignored_inputs.read_manifest(self.root)
...
class TestProvisioning(IgnoredInputsCase):
    ...
    def test_directory_symlink_is_ignored_as_a_link_object(self):
        ...
        if not os.path.islink(link):
            ignored_inputs.pathops.detach_directory_link(link)
            try:
                os.symlink(
        ...
        ignored_inputs.release(self.root, self.worktree)
    ...
    def test_declared_paths_are_provisioned_including_a_nested_one(self):
        ...
        self.assertEqual(ignored_inputs.applied_required_inputs(
            ignored_inputs.read_manifest(self.root), self.worktree),
            contract.required)

    def test_declared_file_is_linked_and_release_preserves_primary_file(self):
        contract = self._review(".env")
        linked = self._link(".env")

        self.assertTrue(os.path.lexists(linked))
        self.assertTrue(os.path.samefile(linked, self.root / ".env"))
        self.assertEqual(linked.read_text(encoding="utf-8"),
                         "TOKEN=local-secret\n")
        self.assertEqual(contract.kinds[".env"], ignored_inputs.FILE_INPUT)
        self.assertEqual(_git(self.worktree, "status", "--porcelain"), "")

        self.assertEqual(ignored_inputs.release(self.root, self.worktree),
                         (".env",))
        self.assertFalse(os.path.lexists(linked))
        self.assertEqual((self.root / ".env").read_text(encoding="utf-8"),
                         "TOKEN=local-secret\n")
    ...
    def test_an_existing_exact_link_is_accepted_and_a_foreign_one_refuses(self):
        ...
        contract = self._classify()
        created, detached = ignored_inputs.reconcile(
            self.root, self.worktree, contract)
        ...
        with self.assertRaisesRegex(AssentError, "is not a link to"):
            ignored_inputs.reconcile(
                self.root, self.worktree,
                ignored_inputs.Decision(ignored_inputs.REVIEWED_REQUIRED,
                                      ignored_inputs.Profile("f", ("assets",)),
                                      {}, ("assets",)),
                manifest=manifest)
        self.assertEqual(
    ...
    def test_profile_switch_preserves_a_recorded_link_until_review(self):
        ...
        ignored_inputs.prepare_worktree(self.root, self.worktree)
        ...
        self.assertTrue(os.path.lexists(self._link("assets")))
        self.assertEqual(
            ignored_inputs.applied_required_inputs(
                ignored_inputs.read_manifest(self.root), self.worktree),
            ("assets", "pkg"))
        with self.assertRaisesRegex(AssentError, "outside its active"):
            ignored_inputs.prepare_sources(
    ...
    def test_a_declared_target_that_disappears_or_changes_type_fails_closed(self):
        ...
        with self.assertRaises(AssentError):
            ignored_inputs.reconcile(self.root, self.worktree, contract)
    ...
    def test_running_the_review_in_the_primary_worktree_links_nothing(self):
        ...
        # to itself.
        self.assertFalse(
            ignored_inputs.pathops.is_link(self.root / "pkg"))

    def test_status_distinguishes_primary_targets_from_source_links(self):
        self._review("pkg", "assets")
        manifest = ignored_inputs.manifest_path(self.root)
        before = manifest.read_bytes()

        output = io.StringIO()
        with contextlib.redirect_stdout(output):
            code = ignored_inputs_status(self.worktree)
        ...
        self.assertIn("Required inputs: assets, pkg", text)
        ...
        output = io.StringIO()
        with contextlib.redirect_stdout(output):
            code = ignored_inputs_status(self.root)
        self.assertEqual(code, 0)
        self.assertIn(
    ...
    def test_release_detaches_recorded_links_and_forgets_the_worktree(self):
        self._review("pkg", "assets")
        detached = ignored_inputs.release(self.root, self.worktree)
    ...
    def test_application_problem_reports_an_altered_link_without_repairing_it(self):
        self._review("pkg")
        self.assertEqual(
            ignored_inputs.application_problem(self.root, self.worktree), "")
        ignored_inputs.pathops.detach_directory_link(self._link("pkg"))
        problem = ignored_inputs.application_problem(self.root, self.worktree)
        self.assertIn("pkg", problem)
        self.assertFalse(os.path.lexists(self._link("pkg")))


class TestIgnoredInputEvidence(IgnoredInputsCase):
    def test_the_digest_covers_declared_target_content(self):
        contract = self._review("pkg")
        before = ignored_inputs.ignored_inputs_digest(
            self.root, [("plan測試", contract)])
        self.assertEqual(
            before,
            ignored_inputs.ignored_inputs_digest(self.root, [("plan測試", contract)]))

    def test_the_digest_covers_a_declared_file(self):
        contract = self._review(".env")
        before = ignored_inputs.ignored_inputs_digest(
            self.root, [("plan測試", contract)])

        (self.root / ".env").write_text("TOKEN=rotated\n", encoding="utf-8")

        self.assertNotEqual(
            before,
            ignored_inputs.ignored_inputs_digest(
                self.root, [("plan測試", contract)]))
    ...
    def test_reviewed_none_is_its_own_identity_and_unknown_has_none(self):
        ...
        empty = ignored_inputs.ignored_inputs_digest(
            self.root, [("plan測試", none_contract)])
        nothing = ignored_inputs.ignored_inputs_digest(self.root, [])
        ...
        no_candidate = ignored_inputs.Decision(
            ignored_inputs.NO_IGNORED_INPUT_CANDIDATE)
        self.assertNotEqual(
            empty,
            ignored_inputs.ignored_inputs_digest(
        ...
        with self.assertRaisesRegex(AssentError, "not a reviewed answer"):
            ignored_inputs.ignored_inputs_digest(
                self.root,
                [("plan測試", ignored_inputs.Decision(ignored_inputs.UNKNOWN))])
    ...
    def test_a_nested_link_is_recorded_by_identity_and_never_followed(self):
        ...
        contract = self._review("pkg")
        first = ignored_inputs.ignored_inputs_digest(
            self.root, [("plan測試", contract)])
...
class TestDeclarationClause(unittest.TestCase):
    def test_unknown_contract_gets_one_validated_cli_instruction(self):
        contract = ignored_inputs.Decision(
            ignored_inputs.UNKNOWN, needs_review=True,
            inventory=("assets", "build"),
            kinds={"assets": "directory", "build": "directory"})

        clause = ignored_inputs.declaration_clause(contract)

        self.assertIn("assent ignored-inputs declare", clause)
        self.assertIn("--not-required PATH REASON", clause)
        self.assertIn("  - assets (directory)", clause)
