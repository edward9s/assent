...
檔名 = `t` + 三位數編號 + `_` + 簡短名稱 + `.toml`。**任務 id = 檔名前綴 tNNN**
(id 只存在於檔名,檔內不重複存)。執行順序 = 檔名字典序。
...
由任務檔的完整主幹加上 `.r.toml` 產生,一任務一檔、**有需要才讀**。
例如 `t001_骨架.toml` 對應 `t001_骨架.r.toml`;兩者依名稱排序時固定相鄰。
...
1. 依**檔名字典序**掃描工作資料夾的任務檔 `tNNN_名稱.toml`,明確排除
   `*.r.toml`(三位數編號保證順序)。
