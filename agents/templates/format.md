# .agents plan format (the agents format contract)

> This file is the single format contract that both "plans produced by an AI
> meeting" and "automatic execution by the agents scheduler" obey.
...
> The objective standard for a valid format: `agents check` passes — which is
...
```text
project/
├── AGENTS.md                    # project rules (root; versioning is the project's call; keep one agents bridge line)
└── .agents/
    ├── agents.toml              # scheduler config: adapter, tier mapping tables
    ├── instructions.md          # agents session instructions and cross-project common rules
    ├── format.md                # this file
    ...
    └── bootstrap01/             # work folder (named after the nature of the work, decided by the meeting, = git branch prefix)
        ...
        └── _report.md           # human-readable report (auto-generated by agents, not versioned)
```
...
- **Parallel execution**: a single work folder allows only one `run` at a time,
  ...
  argument `agents run <folder>` can state the work folder explicitly, and is
  orthogonal to `--config`.
- **Cleanup**: `agents clean [FOLDER]` handles only the fixed-location redundant
...
- **Rejection**: `agents reject <FOLDER>` is an explicit human-adjudicated
  ...
  run is in progress.
- **Rework**: `agents rework <FOLDER> <TASK>` non-destructively reopens a single
...
### Project rules and the agents management plane
...
agents. If it is versioned, the executing AI reads the branch version inside the
...
tool-managed block as the agents session flow.
...
scope = ["agents/", "tests/"]    # allowed path prefixes for changes; fail-closed, must not be empty
...
`agents run [FOLDER]` runs only that folder when the folder is stated
...
`agents run --all` runs all incomplete folders in folder-dependency order;
...
`agents clean [FOLDER]` removes provably redundant worktrees and same-folder
...
`agents reject <FOLDER>` lets a review meeting reject a whole folder's
...
`agents rework <FOLDER> <TASK>` lets a review meeting reopen a single task; both
...
3. `agents check` validates reference integrity: deps point to existing tasks,
...
`agents report` (or run closeout, or after a successful rework) aggregates the
...
- Opening meeting: read AGENTS.md + instructions.md + this file, and settle
  ...
  version must be task files); the adjournment condition = `agents check` passes.
- Review meeting: the human first reads `_report.md` (zero token) and opens an
  ...
  `agents rework <FOLDER> <TASK>`, and state `--cascade` when downstream
  ...
  facts means unfinished, and adjourning still requires `agents check` to pass.
- Project-specific decisions that stay valid across plans settle into AGENTS.md,
  not carried by a closed task file; agents rules common across projects update
...
equivalent: `agents check` passes.
