"""Inspect or declare the project's required ignored inputs.

``assent ignored-inputs declare`` is a deliberately narrow operation. It takes
...
``assent ignored-inputs status`` is the read-only companion.  It classifies the
...
this validated operation is the one way it can settle the ignored-input
decision.

The parser is built here so the surrounding CLI can attach it, and the module
also runs standalone
(``python -m assent.ignored_inputs_cli ignored-inputs declare ...``).
"""
...
Examples:
  assent ignored-inputs declare --required assets --not-required build "build output" --watch package.lock
  assent ignored-inputs declare --none-required --not-required build "build output" --watch package.lock

--required names a project-relative ignored file or directory that the source requires.
--not-required accounts for every inventory input that is not required,
with a reason.
--watch is a repeatable, tracked dependency or build file; changing it makes the
decision stale. Every inventory input must be covered exactly once by
...
def add_ignored_inputs_command(sub: argparse._SubParsersAction) -> None:
    """Attach the read-only status and controlled declaration operations."""
    parser = sub.add_parser(
        COMMAND,
        help="inspect or declare this source's required ignored inputs")
    operations = parser.add_subparsers(dest="operation", required=True)
    operations.add_parser(
        "status",
        help="show this worktree's ignored-input decision and link state",
        description="Inspect this worktree's ignored-input state without changing it.")
    declare = operations.add_parser(
        "declare",
        help="AI workflow: declare this source snapshot's required ignored inputs",
        description="Validate, record, and apply one ignored-input decision.",
        epilog=_DECLARE_HELP,
        formatter_class=argparse.RawDescriptionHelpFormatter)
    declare.add_argument(
        "--required", action="append", default=[], metavar="PATH",
        help="a project-relative ignored file or directory this source requires "
             "(repeatable)")
    declare.add_argument(
        "--none-required", action="store_true",
        help="record that this source requires no ignored input")
    declare.add_argument(
        "--not-required", action="append", nargs=2, default=[],
        metavar=("PATH", "REASON"),
        help="record why one ignored inventory input is not required "
             "(repeatable)")
    declare.add_argument(
...
def ignored_inputs_declare(required: list[str], watch: list[str],
                         none_required: bool,
    ...
    if here.resolve() == main.resolve():
        raise AssentError(
            "ignored-inputs declare is an AI workflow operation and must run "
            "in its managed source worktree")
    decision = ignored_inputs.declare(
        main, here, required=required, watch=watch,
        none_required=none_required,
        not_required=tuple(
            ignored_inputs.NonRequiredInput(path, reason)
            for path, reason in (not_required or [])))
    print(f"Recorded ignored-input profile {decision.profile.fingerprint[:12]} "
          f"in {ignored_inputs.manifest_path(main)}")
    print(ignored_inputs.describe(decision))
    return 0


def ignored_inputs_status(cwd: Path | None = None) -> int:
    """Describe the current worktree's ignored-input decision without changing it."""
    ...
    decision = ignored_inputs.classify(main, here, manifest)

    print(f"Current worktree: {here}")
    print(f"Primary worktree: {main}")
    presence = "present" if manifest.present else "absent"
    print(f"Manifest: {ignored_inputs.manifest_path(main)} ({presence})")
    ...
    if decision.prior_required and decision.profile is None:
        print("Previously required inputs: "
    ...
    if not decision.settled:
        ...
        return 0
    try:
        ignored_inputs.require_input_link_agreement(main, here, decision)
