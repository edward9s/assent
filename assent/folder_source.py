"""Neutral ownership of one work folder's current Git source and finished set.

Accept, clean, reject, and reconcile all answer the same two questions before
they touch anything: which single ``<folder>/*`` branch is the folder's current
...
# The task statuses that make a whole folder finished.  Written once here so
# accept, clean, reject, and reconcile cannot drift apart on what "finished"
# means.
COMPLETE_STATUSES = ("DONE", "SKIP")


def resolve_source_snapshot(main: Path, folder: str,
