...
An older parsed receipt version is invalidated and regenerated by the scheduler
before verification. AI preflight roles never edit receipts. Malformed receipts
and unknown future versions refuse, and acceptance never performs an upgrade.

## Ignored inputs

Complete verification mirrors only reviewed ignored-input links and ordinary
ignored leaf files generated inside otherwise tracked source directories.
Nothing is copied. Ignored directory contents, `.git`, `.assent`, and linked
targets are not enumerated during discovery.

The primary worktree's untracked `.assent/_ignored-inputs.toml` stores reviewed
ignored-input profiles. `assent ignored-inputs declare` is the only writer.
A profile may classify as `UNKNOWN`, `REVIEWED-NONE`, `REVIEWED-REQUIRED`,
`STALE`, or
`NO-IGNORED-INPUT-CANDIDATE`. Unknown or stale evidence adds one bounded
...
Required inputs are immutable role inputs. Before and after every AI role,
...
`declare --required PATH` records a required source input and provisions its
same-relative link: a Windows junction or POSIX symlink for a directory, and a
Windows hard link or POSIX symlink for a file. `--none-required` records that no
inventory input is required. `--not-required PATH REASON` accounts for caches,
output, and every other inventory input. The command is not a general link
manager and never copies an input.
